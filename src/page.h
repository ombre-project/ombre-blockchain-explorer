...
public:
                    ...
                    bool tx_prove = false)
    {
        ...
        {
            if (!tx_prove)
                return show_error_page("Decode outputs", "ombre address not provided!");
            else
                return show_error_page("Prove sending", "ombre address not provided!");
        }
        ...
        // parse string representing given ombre address
        ...
        if (!xmreg::parse_str_address(xmr_address_str,  address_info, nettype))
        {
            ...
            if (!tx_prove)
                return show_error_page("Decode outputs", "Can't parse ombre address: " + xmr_address_str);
            else
                return show_error_page("Prove sending", "Can't parse ombre address: " + xmr_address_str);
        }
...
//        }
        ...
        // we get number of all possibilities, and their total ombre amount
...
//                                << endl;
        ...
        // useful for unit testing as it provides total ombre sum
        // of possible mixins
    ...
    search(string search_text)
    {
        ...
        // check if ombre address is given based on its length
        ...
        if (search_str_length == 99)
        {
            ...
            if(search_text.substr(0,6) == "ShaDoW" ||
               search_text.substr(0,7) == "ShaDowS" ||
               search_text.substr(0,6) == "ShaDoe" ||
               search_text.substr(0,4) == "casi" ||
			   search_text.substr(0,4) == "cash" ||
			   search_text.substr(0,7) == "ShaDoeS" ||
			   search_text.substr(0,6) == "ShaDos")
            {
                nettype_addr = cryptonote::network_type::TESTNET;
            }

            if(search_text.substr(0,5) == "Suto" ||
			   search_text.substr(0,5) == "Suti" ||
			   search_text.substr(0,2) == "Tr" ||
			   search_text.substr(0,2) == "Tu" ||
			   search_text.substr(0,2) == "Tk" ||
			   search_text.substr(0,2) == "Tb" ||
               search_text.substr(0,2) == "Tw")
            {
        ...
        // check if kurz ombre address is given based on its length
        ...
        if (search_str_length == 55)
        {
            ...
            if(search_text.substr(0,4) == "casi" ||
               search_text.substr(0,6) == "ShaDoe")
            {
                nettype_addr = cryptonote::network_type::TESTNET;
            }

            if(search_text.substr(0,2) == "Tr" ||
               search_text.substr(0,2) == "Tu")
            {
                nettype_addr = cryptonote::network_type::STAGENET;
            }
        ...
        if (search_str_length == 110)
        {
            ...
            if(search_text.substr(0,7) == "ShaDowS" ||
               search_text.substr(0,7) == "ShaDoeS")
            {
                nettype_addr = cryptonote::network_type::TESTNET;
            }
 ...
 */
                 ...
                 bool tx_prove = false)
    {
        ...
        {
            j_response["status"]  = "error";
            j_response["message"] = "ombre address not provided";
            return j_response;
        }
        ...
        {
            j_response["status"]  = "error";
            j_response["message"] = "Can't parse ombre address: " + address_str;
            return j_response;
    ...
    json_networkinfo()
    {
        ...
        {
            j_response["status"]  = "error";
            j_response["message"] = "Can't get ombre network info";
            return j_response;
        }

        j_info["fee_per_kb"] = 500000;
